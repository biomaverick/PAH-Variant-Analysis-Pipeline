...
- **FoldX/PyMOL structural stability modeling was scoped out** of this pilot — licensing/installation overhead wasn't justified given that independent clinical classification (ClinGen) and computational pathogenicity predictors (AlphaMissense, ClinPred) already provided strong, literature-grounded functional evidence for the one missense variant identified. This would be a natural next step for a variant without existing clinical characterization.

## Tools used
FastQC, fastp, BWA-MEM, GATK4 (HaplotypeCaller, GenomicsDBImport, GenotypeGVCFs, VariantFiltration), bcftools, samtools, Ensembl VEP (AlphaMissense, ClinPred annotations)

## Future work
- Extend to the second NA12878 replicate (NIST7086) as a reproducibility check
- Structural stability modeling (DynaMut2/FoldX) for any future variants lacking existing clinical classification
